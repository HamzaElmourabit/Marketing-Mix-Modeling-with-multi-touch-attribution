...
def _normalize_treatment_label(value):
    if pd.isna(value):
        return None

    text = str(value).strip().lower()
    if text in {"control", "ctrl", "c", "0", "false", "no", "holdout"}:
        return "control"
    if text in {"treatment", "treat", "test", "1", "true", "yes", "variant"}:
        return "treatment"
    return text
...
def analyze_geo_experiment(
    ...
    working = df.copy()
    working[treatment_column] = working[treatment_column].map(_normalize_treatment_label)

    grouped = (
        working.groupby([geo_column, treatment_column])[metric_column]
        .agg(['sum', 'mean', 'count'])
        .reset_index()
    )
