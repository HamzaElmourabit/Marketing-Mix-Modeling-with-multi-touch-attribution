...
def train_bayesian_mmm(
    ...
    design_cols = ['intercept'] + data['feature_cols']
    ...
    feature_means = {name: float(value) for name, value in zip(data['feature_cols'], data['feature_means'])}
...
def estimate_bayesian_budget_revenue(
    ...
    def _map_spend_to_feature(spend_col: str) -> Optional[str]:
        ...
        base_name = spend_col.replace('_SPEND', '').replace('_ADSTOCK', '').replace('_SAT', '')
        candidates = [
            c for c in feature_cols
            if base_name in c and 'ADSTOCK_SAT' in c
        ]
    ...
    for spend_col, budget in proposed_budget.items():
        ...
        if spend_col in df.columns:
            ...
            current_spend = float(df[feature_name].sum())
        else:
            current_spend = 0.0
